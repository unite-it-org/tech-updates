"""
News Aggregator Agent - HTTP Server
Wraps the news aggregator agent as an HTTP server for production use.
"""
...
CATEGORIES = ["AI", "AUTOMOTIVE", "INFORMATION TECHNOLOGY", "SCIENCE", "ART"]
ARTICLE_COUNT = 15
LANGUAGE = "en"
...
app = FastAPI(title="News Aggregator Agent", version="1.0.0")
...
async def generate_news_html(category: Optional[str] = None) -> str:
    """Generate HTML from news articles."""
    global CATEGORIES
    
    categories_to_fetch = [category] if category else CATEGORIES
    
    # Temporarily set categories
...
async def get_tech_news(
    ...
    """
    return await generate_news_html(category)
...
async def root():
    """Root endpoint."""
    return {"message": "News Aggregator Agent API", "version": "1.0.0"}
...
async def get_news(request: NewsRequest):
    """
    Fetch and aggregate news, returning HTML output.
    Optionally specify a category (AI, Automotive, IT, Science, Art).
    """
    try:
        # Generate HTML
        html_content = await generate_news_html(request.category)
        ...
        # Count articles (approximate)
        article_count = html_content.count("<a href=")
...
async def get_news_by_category(category: str):
    ...
    try:
        html_content = await generate_news_html(category)
...
def create_agent():
    """Create the news aggregator agent using OpenAI SDK with Gemini API."""
